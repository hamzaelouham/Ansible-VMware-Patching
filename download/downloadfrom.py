...
def download_folder(access_token, site_id, drive_id, folder_path, local_path):
    """
    Download only the specified .zip files from the top-level SharePoint folder.
    """

    # Define the exact ZIP files to download
    target_files = {
    ...
    for item in items:
        ...
        # Download only if it's one of the target .zip files
        if "file" in item and item_name in target_files:
            ...
            print(f"🎯 Downloading target ZIP file: {item_name}")
            with requests.get(file_url, stream=True) as response:
                response.raise_for_status()
                with open(file_path, "wb") as file:
                    shutil.copyfileobj(response.raw, file)
        else:
            print(f"Skipping file: {item_name}")

    print(f"✅ Selected ZIP files downloaded successfully from: {folder_path}")
